...
> [!IMPORTANT]
> **Treebery** is a fork of [Sidebery](https://github.com/mbnuqw/sidebery) by [mbnuqw](https://github.com/mbnuqw), ported to Chromium-based browsers on Manifest V3. The sidebar runs in Chrome's side panel. Some Firefox-only features are not available on Chromium and are disabled: containers, hiding tabs of inactive panels, native context menus, theme syncing, proxy/webRequest rules, and the omnibox. All credit for Sidebery goes to the original author; this fork keeps the original MIT license.
>
> Build for Chromium: `npm install && npm run build.chrome`, then load the `addon/` directory via `chrome://extensions` → "Load unpacked".
